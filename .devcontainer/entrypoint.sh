...
# Sync devcontainer scripts from workspace (so rebuild without image rebuild picks up changes)
if [ -n "${WORKSPACE_FOLDER:-}" ]; then
    for script in cargo_compete_new.sh run_pahcer_for_ui.sh cargo_snippet.sh ahc_optuna.sh; do
        cp -f "$WORKSPACE_FOLDER/.devcontainer/$script" "/usr/local/bin/$script"
        chmod +x "/usr/local/bin/$script"
    done
    ln -sfn /usr/local/bin/ahc_optuna.sh /usr/local/bin/ahc_optuna
fi
